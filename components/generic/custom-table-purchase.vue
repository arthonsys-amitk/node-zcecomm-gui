...
<div v-if="total==0" class="container"><h2>No Results Found</h2></div>
 <div v-else>
  <div class="row" style="background-color:black;color:white;padding:10px;">
      <div :class="column.class" v-for="column in columns" :key="column.field"><b>{{column.title}}</b></div>
 </div>


 <div  class="row" v-for="purchase in data" :key="purchase._id">
       <div class="col-sm-3 row-purchase">{{purchase.firstName}}  {{purchase.lastName}}</div>
       <div class="col-sm-3 row-purchase">{{purchase.email}}</div>
       <div class="col-sm-3 row-purchase">{{purchase.email}}</div>
       <div class="col-sm-3 row-purchase"><opts :row="purchase" :xprops="xprops"></opts></div>
      
    
       <div class="col-sm-3  product-header">Name</div>
       <div class="col-sm-3 product-header">Quantity</div>
       <div class="col-sm-3 product-header">Expiration</div>
       <div class="col-sm-3 product-header">Action</div>
      

            <div class="row-product" style="background-color:#ECEFF1;"  v-for="product in purchase.cart" :key="product.productID">
               <div class="col-sm-3" style="padding:5px;">{{product.productName}}</div>
               <div class="col-sm-3" style="padding:5px;">{{product.quantity}}</div>
               <div class="col-sm-3" style="padding:5px;">{{product.productKeyExpiration.slice(0,10)}}</div>
               <div class="col-sm-3" style="padding:5px;"><opts-cancel :row="product"  :xprops="xcancelprops"></opts-cancel></div>
            </div>
   ...
   <modal-component v-if="askproductcancelconfirmation" v-on:accepted="cancelProductConfirmed" v-on:rejected="cancelProductionRejected" modaltype="confirm" modalid="cancelproductconfirmation" displaytitle="canceldisplaytitle" :displaytext="cancelproductdisplaytext"></modal-component>
</div>
...
data: function () {
  return {
    ...
    xprops : {
      showbuttons : this.buttons || {"edit": true, "delete" : true,"cancel":false}, //We can control if we don't want to display any specific action button for whole table.
      eventbus: new Vue()
    },
    xcancelprops : {
